...
def scale_image(image, scale=1, interpolation=cv2.INTER_AREA, scale_values=False):
...
def scale_mvs_input(images, cams, view_num, depths=None, scale=1, modify_intrinsics=False, scale_values=False):
    """ resize input to fit into the memory """
    for view in range(view_num):
        images[view] = scale_image(images[view], scale=scale, scale_values=scale_values)

        if not depths is None:
            depths[view] = scale_image(depths[view], scale=scale, interpolation=cv2.INTER_NEAREST)

        if modify_intrinsics:
            cams[view] = scale_camera(cams[view], scale=scale)
...
def crop_mvs_input(
        images, cams, view_num, depths=None, max_w=0, max_h=0, base_image_size=8, modify_intrinsics=False,
        ):
...
def get_camera_params(intrinsics_file, render_extrinsics_file, args, default_exts=None):
    if '.mat' in intrinsics_file and os.path.exists(intrinsics_file):
        K = scipy.io.loadmat(intrinsics_file)['K']
    else:
        # HACK(mokunev): support for both legacy and current datasets
        if '.mat' in intrinsics_file:
            intrinsics_file = intrinsics_file.replace('.mat', '.npy')
        if os.path.exists(intrinsics_file):
            K = np.load(intrinsics_file)
        else:
            raise ValueError('Intrinsics file does not exist: %s' % intrinsics_file)

    if K.shape[0] == 3:
        # Extend K to 4x4 from 3x3
        eye = np.eye(4)
        eye[:3, :3] = K
        K = eye
