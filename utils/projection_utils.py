...
def get_render_poses_spiral(focal_length, bounds_data, intrinsics, poses, args, N_views=60, N_rots=2, create_spiral=False, spiral_radius=1):
    ...
    # Translates camera poses on camera-space u and v axis
    # spiral_radius controls amount of translation across u and v axis
    # Assumes input_views have constant camera_pose, so we need to manually generate spiral
    if create_spiral :
        center_idx = ((len(poses) / 2)- ((len(poses) / 2) % 4)) + 4
        center_idx = int(center_idx)
        u = poses[center_idx][0, 0:3]
        v = poses[center_idx][1, 0:3]
    
        for i in range(len(poses)) :
            # Maps [0, len] -> [-0.5, 0.5] * radius
            u_step = (float(i) / float(len(poses))) - 0.5
            u_step = u_step * spiral_radius
            v_step = (float(i) / float(len(poses))) - 0.5
            v_step = v_step * spiral_radius

            translated_center = u * u_step + v * v_step + poses[i, 0:3, 3]
            poses[i, 0:3, 3] = translated_center
