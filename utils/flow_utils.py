...
def compute_flow_metrics(source_coordinates, flow_gt, predicted_pixels, dynamic_threshold = 1.0):
    # source_coordinates: [N, 2]
    # flow_gt: [N, 2]
    # predicted_pixels: [N, 2] -- where the flow is predicted to go
    # Output: endpoint error, endpoint error on dynamic pixels only,
    #         relative endpoint error on dynamic pixels only

    # Compute endpoint error
    gt_pixels = source_coordinates + flow_gt
    error = np.linalg.norm(gt_pixels - predicted_pixels, axis=1)
    endpoint_error = np.mean(error)
    flow_gt_len = np.linalg.norm(flow_gt, axis=1)
    dynamic_mask = flow_gt_len >= dynamic_threshold
    if any(dynamic_mask):
        dynamic_error = error[dynamic_mask]
        dynamic_endpoint_error = np.mean(dynamic_error)
        dynamic_rel_error = np.mean(dynamic_error / flow_gt_len[dynamic_mask])
    else:
        dynamic_endpoint_error = 0.0
        dynamic_rel_error = 0.0

    return {
       'epe': endpoint_error, 
       'dynamic_epe': dynamic_endpoint_error, 
       'dynamic_relative_error': dynamic_rel_error
    }
